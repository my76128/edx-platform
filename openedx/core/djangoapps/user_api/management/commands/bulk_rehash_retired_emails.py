...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for retirement in retirements:
            ...
            new_retired_email = user_util.get_retired_email(original_email, settings.RETIRED_USER_SALTS, settings.RETIRED_EMAIL_FMT)

            # Sanity check:
            if retirement.user.email != old_retired_email:
                print(
                    'WARNING: Skipping UserRetirementStatus ID {} / User ID {} because the user does not appear to have a '
                    'retired email address: {}.'.format(
                        retirement.id,
                        retirement.user.id,
            ...
            # If the original email address was already normalized (or all lowercase), the old and new hashes would match:
                ...
                print('Skipping UserRetirementStatus ID {} / User ID {} because the email hash would not change.'.format(
                    retirement.id,
                    retirement.user.id,
                ))
